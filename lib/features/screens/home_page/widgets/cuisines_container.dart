...
class CuisinesTile extends StatelessWidget {
  const CuisinesTile({super.key, required this.list});
      ...
      child: GestureDetector(
        onTap: () {
          BlocProvider.of<ListCubit>(context)
              .getCusine(cusine: list.CuisinesName);
          Navigator.of(context).push(
            MaterialPageRoute(
              builder: (context) {
                return BlocBuilder<ListCubit, ListState>(
                    builder: ((context, state) {
                  if (state is CusinesLoadingState) {
                    return Center(child: CircularProgressIndicator());
                  } else if (state is CusinesLoadedState) {
                    return CusinesScreen(
